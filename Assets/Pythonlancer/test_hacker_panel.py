...
XML_SOLAR_MAT_TEMPLATE = '''<UTFXML filename="solar_mat_hacker.mat">
   <UTF_ROOT>
...
MATERIAL_TEMPLATE = '''
 <hacker{index:02d}>
    <Dc type="RGB">{R}, {G}, {B}</Dc>
    <Type type="text">DcDtEc</Type>
    <Ec type="RGB">{R}, {G}, {B}</Ec>
 </hacker{index:02d}>
'''

XML_MAIN_TEMPLATE_OLD = '''<?xml version="1.0" encoding="ISO-8859-1"?>
...
class HackerButton(object):
    ...
    def get_part_fix(self, appear_valid):
        ...
        if (appear_valid and self.valid) or (not appear_valid and not self.valid):
            pos_y = VISIBLE_DRIFT + (self.layer.layer_index * LAYER_DRIFT) - self.layer.layer_index
    ...
    def get_collision_group(self):
        return COLLISION_GROUP_TEMPLATE.format(index=self.layer_button_index)
