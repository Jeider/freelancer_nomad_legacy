...
from story.actors import Trent, Hatcher, Darcy, Hassler, Yamamoto, Kim, SakuraOne, SakuraTwo, SakuraThree, Chrysanthemum, Matome
    ...
    VOICE_LINES = [
        ...
        VoiceLine(200, Kim, comment='Операция',
                  ru='Мистер Трент, я лейтенант Ким. Я буду руководить операцией. '
                     'А это Матомэ, он будет управлять инженерными силами'),
        VoiceLine(210, Kim,
        ...
        VoiceLine(210, Kim,
                  ru='Нам нужно добраться в место сбора в системе Сириус. Я отметил точки пути в вашей нейросети'),
        #
        # 'Это большая честь участвовать в операции вместе с вами, мистер Трент. У вас благородная репутация. Хоть вы и работали на СБА'
        #
        # 'А чем вам не нравится СБА? Вроде же ребята официалы, с пиратами не сотрудничают. В отличие от...'
        #
        # 'Мы понимаем ваши намёки, мистер Трент. Орден занимается этим. После нашего прихода во внешние миры наконец добралась цивилизация'
        # 'Мы занимаемся инфраструктурой, терраформировали Кадиз и искоренили производство Кардамина. Теперь испанцы живут лучше'
        #
        # 'Но это все равно не мешает корсарам продолжать совершать набеги и терроризировать Сириус'
        # 'Да и проблему наркотиков в целом это не решило. А то я знаю знакомых принцесс, которые любят припудрить свой носик. На регулярной основе'
        #
        # 'Всему своё время, мисс Дерси. Мы не можем решить все противоречия сразу. Особенно в то время, когда СБА проводит такую политику протекционизма'
        # 'СБА больше заинтересованы в защите мифического ящика Пандоры вместо того, чтобы реально помогать людям и решать их проблемы'
        #
        # 'Очень хочется надеяться, что вы действительно говорите правду'
        #
        # 'Не сомневайтесь, мистер Трент. Тем более, что сегодня вы нам с этим поможете'
        #
