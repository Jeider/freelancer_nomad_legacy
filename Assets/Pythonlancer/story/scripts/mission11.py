...
class Msn11DrinkCutscene(Msn11, script.CutsceneProps):
    ...
    VOICE_LINES = [
        ...
        VoiceLine(140, Trent, ru='Я устал, нет серьезно, достало всё. Берешь простой заказ, а тебе вместе с оплатой впаривают человека, которому ну просто необходимо пом+очь.',
                  en='I\'m so tired. Seriously, I\'ve had enough. You take a simple job, and it comes with a side of some guy who desperately needs your help.'),
        VoiceLine(150, Trent, ru='Он обещает тебе златые горы - и вот, трах-бах, и ты уже поуши увяз в интригах спецслужб, и за твою голову назначена неплохая такая цена.', en='He promises you the world—and then, bam! You\'re neck-deep in some intelligence agency mess with a hefty bounty on your head.'),
        VoiceLine(160, Trent, ru='И ты пытаешься из всего этого выпутаться, но вместо этого запутываешься еще больше.', en='And you try to find your way out of this mess, but you only dig yourself a deeper hole.'),
        VoiceLine(170, Trent, ru='И в конце концов, ты уже не понимаешь за кого и против кого ты, а самое главное - зачем это всё?', en='In the end, you can\'t even tell friend from foe anymore. And the most important question is: what\'s the damn point of it all?'),

        VoiceLine(180, EdisonTrent, ru='Так всегда и бывает,, тёзка, ты же фрилансер. Либо ты отстреливаешь мелких бандюков в свободных мир+ах, летая на ржавом корыте и получаешь за это копейки. Либо вот так...', en='That\'s how it always is, namesake. You\'re a freelancer. You either take the small jobs: popping two-bit thugs in the outer systems, flying a rusty can for scrap wages... Or you end up in a mess like this.'),
        ...
        VoiceLine(210, EdisonTrent, ru='И,, самое главное, врагов,, у тебя конечно становится всё больше и больше, но у тебя появляются друзья.', en='And most importantly... sure, you\'ve got more and more enemies. But you\'ve also made friends.'),
        ...
        VoiceLine(230, EdisonTrent, ru='И ты уверен, что бы ни случилось, на них можно положиться. И вот эти друзья, они на самом деле намного ценнее, чем шестизначный счет или новейший корабль с лучшим обвесом.', en='And you know, no matter what happens, you can count on them. And these friends... they\'re actually worth far more than any six-figure account or a state-of-the-art ship with the best gear.'),
        VoiceLine(240, EdisonTrent, ru='Вот так. А пожалеть себя иногда очень хочется, по себе знаю. Главное не злоупотреблять этим - вредно для формы.', en='That\'s how it is. And feeling sorry for yourself is a powerful temptation, I know that firsthand. The trick is not to make a habit of it—it\'s bad for your edge.'),
